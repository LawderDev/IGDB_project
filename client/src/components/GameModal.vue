              ...
              <div class="sm:flex sm:items-start text-md">
                ...
                <div class="mt-3 sm:mt-0 sm:ml-4 sm:text-left">
                  <div class="border-b border-gray-400">
                    <DialogTitle as="h3" class="text-xl leading-6 font-medium"> {{ state.name }} </DialogTitle>
                    <p>{{ state.date }}</p>
                    <p class="italic">{{ state.author }}</p>
                    <p><span class="text-slate-400">Support: </span>{{ state.platforms }}</p>
                    <p><span class="text-slate-400">Genres: </span>{{ state.genre }}</p>
                  </div>
                  <div class="mt-2">
                    <p>
                      <h4 class="text-slate-400">Synopsys:</h4>
                      Horizon Forbidden West continues Aloy’s story as she moves west to a far-future America to brave a majestic, but dangerous frontier where she’ll face awe-inspiring machines and mysterious new threats.</p>
                  </div>
                </div>
              </div>
